...
### Exploring Neighbors:
- For each cell, we explore its neighbors (up to 8 neighbors, constant time operation).
- Checking and updating the queue for each neighbor takes $$O(\log N^2) = O(\log N)$$ time.
- Total neighbor exploration in the worst case is $$O(N^2 \cdot 8) = O(N^2 \cdot 8) = O(N^2)$$.
